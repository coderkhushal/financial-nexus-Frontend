...
const ChatWidget = () => {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        setchats(value=>[...value, { message: values.message, message_by: "user" }])
        const idtoken = await auth.currentUser?.getIdToken()
        if (idtoken) {
            console.log(idtoken)

            let response = await axios.post(SERVER + "/data-add/add-message/", {
    ...
    return (
        <div className={`${pathname == "/main/chat" && "hidden"} h-full flex items-end py-2`}>
            <Button onClick={toggle} className={`${show && "hidden"} z-40 absolute aitom-3 right-2`}>chat</Button>
            <div id="chat-container" className={`fixed ${!show && "hidden"} aitom-5 z-40 right-4 w-96`}>
                <div className="bg-white shadow-md pb-2 rounded-lg max-w-lg w-full">
                    <div className=" border-b p-4 bg-gray-900 text-white rounded-t-lg flex justify-between items-center">
                        ...
                        <button id="close-chat" className="text-gray-300 hover:text-gray-400 focus:outline-none focus:text-gray-400">
                            <svg onClick={toggle} xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
